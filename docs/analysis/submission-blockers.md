...
A method-of-moments correction subtracts finite-opportunity noise from variance across prompt means. The primary 50,000-replicate bootstrap retains all sixteen fixed prompts and resamples episodes within each prompt. A separate two-stage bootstrap resamples prompts and is labeled exploratory persona-population uncertainty.
...
All four fixed-panel lower bounds exceed the historical `0.75 × published human SD` threshold. Three of four exploratory persona-population lower bounds do. Neither result is a protocol-matched estimate of human latent heterogeneity.

Artifact: `submission/variance-correction.md`.

### A4. Prompt-indexed response figure — **COMPLETE**

Figure 1 reports each prompt's observed continuation-probability difference with conservative exact simultaneous intervals that remain non-zero-width at corners. Aggregate fixed-panel estimates and intervals are included:

- S2 absent: +0.083, approximately [−0.171, +0.330];
- S2 present: +0.078, approximately [−0.181, +0.330].

The caption states that these are prompt-indexed couplings, not person-level effects without latent-person invariance, and that the point estimates do not establish equivalence.

Artifacts: `docs/paper/figures/prompt-indexed-delta.svg` and `.csv` (generated by `scripts/generate_prompt_delta_figure.py`).
