...
> Dal Bó & Fréchette 2011, R=40 first-round cooperation 0.6110 (published
> δ=.90 column) and 0.1872 (published δ=.10 column); between-subject SDs
> 0.3116 / 0.4122. **Published nonmatched comparator** — the source
> treatment was between-session, so no matched within-subject δ-response
> exists in that design (R2 relabel, 2026-07-29; see
> `docs/analysis/claim-dependencies.md`).
...
The pool's *levels* fall within ~0.15 of the published values and its
*spread* inside the published bands — coarse marginal checks (P5-1b
consistent in all four cells) — while the pool δ-response is
**substantially smaller than the published human δ-gap** (−0.08 vs
−0.42, nonmatched comparator). The S2 paraphrase, by contrast, moves the pool
