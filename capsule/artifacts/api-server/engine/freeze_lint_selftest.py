...
def main() -> int:
    ...
    fec_full = {"P5-1a": "single-candidate: none needed",
                "P5-1b": "Bonferroni over the 4 matched rep cells"}
    r = _run(_manifest(verdictBranches=vb_full, familyErrorControl=fec_full))
    results.append(("A10c fully enumerated axes pass", r["pass"], r))

    # A11 — R2 rule (first post-adjudication downgrade): a manifest sealing
    # predicates without any family-level error-control declaration must
    # fail the freeze. (Reproduces the P5-3 clause-(a) 32-candidate family
    # audited only per-candidate.)
    r = _run(_manifest(verdictBranches=vb_full))
    results.append(("A11 missing familyErrorControl section fails closed",
                    not r["pass"] and
                    any(f["check"] == "C8-family-error-control"
                        for f in r["failures"]), r))
    # A11b — a predicate with an empty/absent declaration fails.
    r = _run(_manifest(verdictBranches=vb_full,
                       familyErrorControl={"P5-1a": "single-candidate: none "
                                                    "needed", "P5-1b": "  "}))
    results.append(("A11b predicate without family declaration fails",
                    not r["pass"] and
                    any(f["check"] == "C8-family-error-control" and
                        f.get("cell") == "P5-1b" for f in r["failures"]), r))
    # A11c — control: full declarations pass (covered jointly with A10c but
    # asserted separately so C8 regressions are attributed).
    r = _run(_manifest(verdictBranches=vb_full, familyErrorControl=fec_full))
    results.append(("A11c full family declarations pass", r["pass"], r))
