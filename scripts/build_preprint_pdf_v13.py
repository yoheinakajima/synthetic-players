...
ROOT=Path(__file__).resolve().parents[1]; PAPER=ROOT/'docs/paper'; SOURCE=PAPER/'paper-draft.md'; HEADER=PAPER/'preprint-v13-header.tex'; OUTPUT=PAPER/'synthetic-players-preprint-v13.pdf'; SHA_FILE=PAPER/'synthetic-players-preprint-v13.sha256'; MANIFEST=PAPER/'synthetic-players-preprint-v13-artifact.json'; BUILD=ROOT/'.preprint-v13-build'; BUILD_MD=BUILD/'synthetic-players-preprint-v13.md'; FIGURES=('prompt-indexed-delta','condition-means','between-prompt-share','representation-effects','p13-audit')
def req(n):
    if shutil.which(n) is None: raise RuntimeError(f'required program not found: {n}')
def git(*args): return subprocess.check_output(['git',*args],cwd=ROOT,text=True).strip()
def prepare():
    lines=SOURCE.read_text(encoding='utf-8').splitlines(); title=lines[0][2:].strip().replace('"','\\"'); body='\n'.join(lines[1:]).lstrip(); body=re.sub(r'^\*\*Preprint v13.*?\n\n','',body,count=1,flags=re.S)
