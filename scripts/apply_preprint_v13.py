...
def main()->int:
    ...
    b=a['stratifiedPromptClusterBootstrap']; d=a['fixedPanelDirichletJeffreys']
    text=PAPER.read_text(encoding='utf-8')
    text=text.replace('**Preprint v12 (July 2026).**','**Preprint v13 (July 2026; Claude review candidate).**')
...
Under the pooled independent-binomial component model this union-bound construction has at least 95% simultaneous coverage; its role is a conservative small-sample projection rather than a claim that the sixteen prompt propensities are homogeneous. It does not assume seat independence and does not collapse to zero uncertainty when all observed episodes agree.'''
    if old not in text: raise RuntimeError('CP paragraph anchor missing')
    text=text.replace(old,new)
    text=text.replace('The frozen ratio ρ=.75 gives thresholds 0.3092 and 0.2337.','The frozen ratio ρ=.75 gives thresholds 0.3092 and 0.2337. The ratio was a preregistered heuristic tolerance—not a theoretically derived equivalence or psychometric margin—so P5-1b is interpreted as a permissive historical dispersion checkpoint rather than evidence of human-variance equivalence.')
    text=text.replace('For comparison, finite-opportunity plug-in shares are 85.5%, 96.1%, 88.8%, and 90.2%.','For comparison, finite-opportunity plug-in shares are 85.5%, 96.1%, 88.8%, and 90.2%, with conditional episode-bootstrap 95% intervals [65.6%, 97.9%], [89.9%, 100.0%], [77.3%, 98.1%], and [77.4%, 98.9%], respectively.')
    ...
    newp=f'''The pooled P5-2 task-consistent share is 90/704 seat decisions across 352 episodes, equivalently 45/352=0.128 on episode means. The frozen historical adjudication used the seat-level rule. An episode-iid Clopper–Pearson projection gives [0.092, 0.172], but that interval does not propagate the prompt clustering visible elsewhere in the study. Two post-adjudication, zero-call sensitivities retain the historical point estimate while changing the uncertainty model: a stratified prompt-cluster bootstrap over the forty registered persona × conflict-cell clusters gives 95% interval [{b['interval95'][0]:.3f}, {b['interval95'][1]:.3f}], and a fixed-panel Dirichlet–Jeffreys latent-propensity aggregation gives posterior median {d['posteriorMedian']:.3f} with 95% interval [{d['interval95'][0]:.3f}, {d['interval95'][1]:.3f]. Both remain below the registered 0.20 persona-dominant boundary, although the Bayesian result approaches it. Every repeated conflict subcell is mixed; only the swap cell is individually persona-dominant. The pooled classification is therefore mechanism-confounded and carried by the swap cell, not evidence of a general persona-dominance mechanism.'''
    if oldp not in text: raise RuntimeError('P5-2 paragraph anchor missing')
    text=text.replace(oldp,newp)
    text=text.replace('The primary evidence comes from one deployment and sixteen complete prompt bundles.','The confirmatory predicates were registered and adjudicated separately at nominal thresholds; no study-wide alpha allocation or familywise rule was registered across P5-1a, P5-1b, P5-2, P5-3(a), P5-3(b), and the sequential X1 extension. They address distinct estimands and are not interpreted here as one omnibus test, but study-level false-positive exposure is consequently greater than under a prospectively hierarchical or alpha-spending design. A Phase 6 replication should preregister the primary/secondary hierarchy, candidate families, dependence units, maximum statistics, and cross-predicate error allocation before data collection.\n\nThe primary evidence comes from one deployment and sixteen complete prompt bundles.')
    text=text.replace('Phase 4–5 contain 30,421 normal request events and 30,397 response events; the 24-event difference is the disclosed provider-failure partial set.','Phase 4–5 contain 30,421 normal request events and 30,397 response events; the 24-event difference is the disclosed provider-failure partial set. Each partial was recorded on a failed run, excluded from completed-run analyses and the 4,916-run replay denominator, and never decoded as an action; replacement runs used fresh registered replacement seeds where required. Request and retry attempts remain represented in request/budget accounting rather than being silently discarded.')
    text=text.replace('Diamonds show fixed-panel aggregates.','Blue and orange diamonds show the S2-absent and S2-present fixed-panel aggregates, respectively.')
    text=text.replace('Preprint v12','Preprint v13')
    PAPER.write_text(text,encoding='utf-8')
    reviews=ROOT/'docs/reviews'; reviews.mkdir(parents=True,exist_ok=True)
    (reviews/'round-11-explore-science-v12-review.md').write_text('# Round 11 - Explore Science review of preprint v12\n\nSource: Explore Science review report dated July 31, 2026 (96/100, Platinum; eight minor issues). The v13 response is additive and zero-call. Historical predicates and verdicts are unchanged.\n',encoding='utf-8')
    (reviews/'round-11-disposition-matrix.md').write_text('''# Round 11 disposition matrix - v13
